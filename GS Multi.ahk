...
NonHostModeFunction() { 
    ...
    if (!condition2) {
        condition2 := True
        GuiControl, Status:, StatusText, Status: cod2
        Mousemove 718, 810
        Sleep, 1000
        Click 718, 810
        Sleep, 1000 ; Wait for 1 second to simulate the click delay
    ...
    if (!condition3 && condition2) {
        condition3 := True
        GuiControl, Status:, StatusText, Status: notcod3cod2
        mousemove 935, 980
        Sleep, 1000
        Click 935, 980
    ...
    if (!condition4) {
        ...
        if (color = 0xA4A6A4) {
            GuiControl, Status:, StatusText, Status: notcod4andyescod3
            MouseMove, 943, 666
            
            ; Start clicking for 4 seconds
            ClickCount := 0
            StartTime := A_TickCount
            
            while (A_TickCount - StartTime < 4000) { ; Loop for 4000 milliseconds (4 seconds)
                Click
                ClickCount++
                Sleep, 100 ; Optional: Adjust sleep time between clicks if needed
            }
            Sleep, 1000
            MouseMove, 976, 827
            Click
