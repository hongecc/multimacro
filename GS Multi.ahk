...
NonHostModeFunction() { 
    global
    ; Check if the pixel at (907, 656) is color 
    if (!condition1) {
        PixelGetColor, color, 907, 656, RGB
        if (color = 0xFF8031) {
            GuiControl, Status:, StatusText, Status: Clicking on coordinates (907, 656)
            Click 907, 656
    ...
    PixelGetColor, colorCheck, 963, 837, RGB
    if (colorCheck >= 0x844000 && colorCheck <= 0x844FFF && !condition2) {
        GuiControl, Status:, StatusText, Status: Clicking on coordinates (880, 810)
        Click 880, 810
        Sleep, 1000 ; Wait for 1 second to simulate the click delay
    ...
    if (!condition3) {
        PixelGetColor, color, 1090, 1000, RGB
        if (color = 0x010202) {
            GuiControl, Status:, StatusText, Status: Clicking on coordinates (1090, 1000)
            Click 1090, 1000
            Sleep, 1000 ; Wait for 1 second to simulate the click delay
            condition3 := True
        }
    ...
    if (!condition4 && condition3) {
        PixelGetColor, color, 1050, 217, RGB
        if (color = 0xE1E0E1) {
            GuiControl, Status:, StatusText, Status: Clicking on coordinates (1050, 217)
            Mousemove, 1099, 668
            Click
            Sleep, 1000
            Loop
                {
                    Click
                    PixelGetColor, color, 1097, 831, RGB
                    if (color = 0xB6B3B6) {
                        GuiControl, Status:, StatusText, Status: Clicking on coordinates (1097, 831)
                        Mousemove, 1097, 831
                        Click
                        Sleep, 100
                        condition3 := False
